...
module.exports = function( options ){
  ...
  // const absBase = path.resolve(options.base || '');
  // const absDest = path.resolve(options.dest || '');
  ...
  function resolveUrlPath( urlFilePath ) {
    var filePath = path.normalize( options.base +'/'+ urlFilePath );

    if ( mm( filePath, options.src ).length === 0 ) return ;
    filePath = path.resolve(options.base +'/'+ urlFilePath);

    var outputFilePath = path.resolve(options.dest +'/'+ urlFilePath);
